    ...
    (apply #'append
	   (loop for y below (cdr fill-size)
		 collect (loop for x below (car fill-size)
			       collect (vec+ fill-offset (cons x y)))))))
...
(defun connect (r1 r2)
  (let* ((rooms (append (car r1) (car r2)))
	...
	(let* ((region2 (pos-flatten r2))
	       (start (randnth (pos-flatten r1)))
	       (end (get-closest-point-to start region2))
	       (dx (- (car end) (car start)))
	       (dy (- (cdr end) (cdr start))))
	  (flet ((region-member (point) (member point region :test 'equal)))
	    (let ((new-corridor (append
				 (unless (eq (cdr start) (cdr end))
				   (flet ((get-point (y)
					    (cons (car start)
						  (if (< dy 0)
						      (- (cdr start) y)
				     ...
				     (loop for y from 0 to (abs dy)
					   unless (region-member (get-point y))
					     collect (get-point y)
					   finally (setf start (cons (car start)
								   (cdr end))))))
				 (unless (eq (car start) (car end))
				   (flet ((get-point (x)
					    (cons (if (< dx 0)
						      (- (car start) x)
						      (+ (car start) x))
