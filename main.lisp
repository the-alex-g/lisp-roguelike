...
(defgeneric movement-cost (object)
  ...
  (:method ((terrain symbol))
    (or (gethash terrain *terrain-costs*) 1))
  (:method ((creature creature))
    (idle-time creature))
  (:method (object) 0))
...
(defgeneric damage (defender amount types &optional statuses)
  (:method ((defender creature) amount types &optional statuses)
    (let* ((base-damage (max 1 (- amount (armor defender))))
      ...
      (max 0 real-damage)))
  (:method ((defender breakable) amount types &optional statuses)
...
(defgeneric attack (defender attacker)
  ...
  (:method :after (defender (attacker player))
    (mapc (lambda (weapon)
	  ...
	  (weapons attacker)))
  (:method ((defender breakable) (attack attack))
    (print-to-log "~a hit ~a for ~d damage~:[~;, ~a~]"
		  (attack-source attack)
		  (name defender)
		  (damage defender (attack-dmg attack) nil)
		  (when (deadp defender) (kill defender) t)
		  (death defender)))
  (:method (defender (attacker creature))
